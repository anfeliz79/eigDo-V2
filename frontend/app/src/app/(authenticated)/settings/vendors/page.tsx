          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Proveedor QBO</th>
                ...
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Ret. ITBIS</th>
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Ret. ISR</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {mappings.map((m) => (
                <tr key={m.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600 text-right">{m.retentionItbisRate ? `${(m.retentionItbisRate * 100).toFixed(0)}%` : '—'}</td>
                  <td className="px-6 py-4 text-sm text-gray-600 text-right">{m.retentionIsrRate ? `${(m.retentionIsrRate * 100).toFixed(0)}%` : '—'}</td>
